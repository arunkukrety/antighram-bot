"""
gui/app_window.py — AgyBotApp: hidden CTk root, system tray, control panel.

Tabs: Status (power toggle / uptime), Config (env editor), Logs (live tail).
The root window only appears on user request; otherwise the app lives in
the system tray.
"""
...
from gui.env_store import read_env, write_env_all
from gui.paths import ENV_FILE, PROJECT_DIR
from gui.startup import get_startup_enabled, set_startup_enabled
from gui.tray_image import make_tray_image
...
class AgyBotApp:
    ...
    def _setup_tray(self) -> None:
        img_off = make_tray_image(running=False)
    ...
    def _update_tray_icon(self, running: bool) -> None:
        ...
        self._tray_icon.icon = make_tray_image(running=running)
